...
class RegisterMap(val dataWidth: Int, val addressWidth: Int) {
  ...
  def createAddressableRegister[T <: Data](register: T, regName: String): Unit = {
  ...
  def addRegister(name: String, width: Int, readCallback: UInt => UInt, writeCallback: (UInt, UInt) => Unit): RegisterDescription = {
